...
**关键观察：合并两条链表的扩展 + 每次取最小头节点的策略**

本题是 [No.21 合并两个有序链表](./No.021_合并两个有序链表.md) 的推广。核心问题变为：**如何在 k 条链表的头节点中，高效选出当前最小值？**

四种策略对应四种"选最小"的方式：

- **顺序合并**：每次取前 k-1 条合并结果与第 k 条做二路归并——等价于 k-1 次二路归并，但长链被反复扫描。
- **分治归并**：两两配对合并，每轮链表数减半——每个节点只参与 O(log k) 次比较，是此题的最优解之一。
- **有序数组（优先队列）**：维护一个始终有序的数组存放各链表头节点，取最小 O(1)，插入新头 O(k)。整体 O(N·k)，适合 k 很小的情况。
- **最小堆**：用堆维护 k 个链表头，取最小 O(1)，插入新头 O(log k)。整体 O(N log k)，是实际工程中最常用的方案。
...
- [No.21 合并两个有序链表](./No.021_合并两个有序链表.md)：本题的母题——二路归并。分治解法和顺序解法都依赖它。
- [No.88 合并两个有序数组](../ch01array/No.088_合并两个有序数组.md)：数组版的二路归并，从尾部填充避免元素移动，思路可类比链表的 dummy 节点技巧。
- [No.148 排序链表](https://leetcode.cn/problems/sort-list/)：对单链表做归并排序，分治合并的过程与本题解法 2（分治归并）几乎一致。
