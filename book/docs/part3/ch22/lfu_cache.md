...
LFU 是 Least Frequently Used 的缩写，即最近最少使用频次淘汰，也是一种常见缓存淘汰机制，核心逻辑是谁被访问次数少，先淘汰谁。

所以，对每个缓存数据都带有一个访问计数器。



根据 LFU 的策略，每访问一次都需要更新该数据的访问计数器，并把数据移到按访问计数器从大到小排序的合适位置。
...
插入数据 D，发现缓存中没有 D，插入 D 将导致容量超过，那么计数器为12的数据 C，会被淘汰；数据 D 放在数据 B 的后面，计数器为1。
...
LFUCache 的 Get 操作，涉及 frequency 的更新和 map 的调整。

具体为：

1. 从 cache map 中查找，如果不存在直接返回
2. 如果存在，将该节点从其 frequency 所在的链表中删除，并移动到 `frequency+1` 的链表中，如果链表不存在就新建 1 个
3. 更新 min 值。如果本次移动的节点 老的 frequency 等于 min，那么移动节点可能造成链表为空，如果为空，min 就必须更新为该节点的新 frequency。
