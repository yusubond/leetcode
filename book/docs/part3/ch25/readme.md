...
给定一个数组array[0....n-1]，实现两个函数：1）求前`i`项和；2）更新第`i`项值。
...
说明：图中数组 A 为原始数据，B 为树形数组。实线表示数组 B 中节点之间的关系；虚线表示两个数组下标对齐。

树形数组的底层结构是数组，但每个节点有树的关系。如图所示，树形数组中父子节点具有这样的关系：
...
有了上面的介绍，我们就可以从原始数组 A 初始化树形数组B，代码如下：
...
查询操作如此高效，得益于这样一个事实：**任意正整数都可以表示为多个2的幂次方之和**。
...
如上介绍，树形数组 B 中父子节点的关系是`parnet = son + 2^k`，那么可以通过这个公式，从叶子节点不断向上递推，直到到最大的父结点为止。
...
单点更新，一般是增量更新，直接 `add(index, val)`即可，如果是全量更新，树形数组结构中多维护一个原始数据，全量更新可以转化为增量更新。
...
- 第 307 题：[区域和检索，数组可修改](./No.307_区域和检索_数组可更新.md)。
